...
class GMM:
    def __init__(self, n=10, ndim=3, record=False):
        self.n = n
        self.ndim = ndim
        self.record = record
        self.refresh()
    ...
    def refresh(self):
        ...
        self.cov = [np.random.normal(0.015, 0.003, size=self.ndim)
    ...
    def __call__(self, x):
        ...
        cst = (2 * 3.14159) ** (- self.ndim / 2)
        modes = np.array([1/np.prod(cov * 10) for cov in self.cov])
...
def test_tf():
    ...
    with tf.Session() as sess:
        gmm = tf_GMM(batch_size=1, ncoef=20, num_dims=2, cov=0.15)
